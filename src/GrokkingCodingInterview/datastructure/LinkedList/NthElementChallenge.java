...
class NthElementChallenge {
    public static <T> Object nthElementFromEnd2(SinglyLinkedList<T> list, int n) {
        int s = list.size;
        int count=0;
        SinglyLinkedList.Node res = null;
        SinglyLinkedList.Node cur = list.headNode;
        while(count<(s-n)){
            cur = cur.nextNode;
            count++;
        }
...
//    }
    public static void main( String args[] ) {
        ...
        System.out.println("3rd element from end : " + nthElementFromEnd2(sll, 3)); //will return null
        for(int i=0; i<15; i+=2){
            sll.insertAtHead(i);
        }
        sll.printList(); // List is 14 -> 12 -> 10 -> 8 -> 6 -> 4 -> 2 -> 0 -> null
        System.out.println("3rd element from end : " + nthElementFromEnd2(sll, 3)); //will return 4
        System.out.println("10th element from end : " + nthElementFromEnd2(sll, 10));//will return null
    }
}