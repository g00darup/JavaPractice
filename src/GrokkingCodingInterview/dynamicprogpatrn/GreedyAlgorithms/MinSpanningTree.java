...
0 -- 3
0 -- 1
Before we move on, let’s have a look at what a minimum spanning tree is.
...
This is a planar graph and its minimum spanning tree. Each edge is labeled with its weight, which here is roughly proportional to its length.

Two different minimum spanning trees of the given graph
Two different minimum spanning trees of the given graph
The figure above shows that there may be more than one minimum spanning tree of a graph. In the figure, two trees below the graph are two possibilities of the minimum spanning tree of the given graph.

To implement a function to find the minimum spanning tree of a graph, we have to make certain changes in our original Graph class. We have added a class that would represent an Edge between two vertices. This includes the addition of the weight property, which will be assigned to every edge. We have also added a class to represent a disjoint set.
 */
public class MinSpanningTree {
    public static void KruskalMST(Graph2 g) {

        Graph2.Edge answer[] = new Graph2.Edge[g.Vertex];
        int j = 0; //index for keeping track of asnwer[]
        int i = 0; //index for keeping track of sorted edges
        for (i = 0; i < g.Vertex; ++i) {
            answer[i] = new Graph2.Edge();
        }
        //sort all edges
        Arrays.sort(g.edge);
        //allocating memory to create subsets
        Graph2.DisjointSets mySet[] = new Graph2.DisjointSets[g.Vertex];
        for (i = 0; i < g.Vertex; ++i)
            mySet[i] = new Graph2.DisjointSets();
        //creating subsets
        ...
        while (j < g.Vertex - 1) {
            ...
            Graph2.Edge nextEdge = new Graph2.Edge();
    ...
    public static void main(String[] args) {
        ...
        MinSpanningTree.KruskalMST(graph);
        System.out.println();


        V = 6;
        E = 15;
        Graph2 graph1 = new Graph2(V, E);
        ...
        MinSpanningTree.KruskalMST(graph1);
    }
